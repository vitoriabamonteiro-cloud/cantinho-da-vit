...
import { fetchCalendarEvents, getWeekRange, getMonthRange, formatTime, formatDateShort, formatDateFull, isSameDay } from "./calendar";

const GOOGLE_CLIENT_ID = "153927080521-0bqo2buqm8lb0codgdadlgs89pi7a31e.apps.googleusercontent.com";
...
function AgendaPage({ accessToken }) {
  ...
  useEffect(() => {
    ...
    fetchCalendarEvents(accessToken, start, end).then(evs => {
      setEvents(evs);
      setLoading(false);
    });
  ...
  if (!accessToken) return <EmptyState icon="📅" text="Conecte sua conta Google para ver sua agenda" />;
    ...
    <div>
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: 40, fontFamily: font, color: palette.textMuted }}>
          <div style={{ fontSize: 36, marginBottom: 8, animation: "pulse 1.5s infinite" }}>📅</div>
          Carregando eventos...
        </div>
      ) : (
        <Card style={{ padding: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4, marginBottom: 8 }}>
            {dayNames.map(d => (
              <div key={d} style={{ textAlign: "center", fontFamily: font, fontSize: 12, fontWeight: 700, color: palette.textMuted, padding: "6px 0" }}>{d}</div>
            ))}
          </div>
          {/* Calendar grid */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4 }}>
            ...
            {Array.from({ length: daysInMonth }, (_, i) => {
              ...
              return (
                <div key={day} style={{
                  minHeight: 70, borderRadius: 12, padding: "6px 8px",
                  background: isToday ? palette.lilac : palette.bg,
                  border: isToday ? `2px solid ${palette.lilacDark}` : `1px solid ${palette.border}`,
                  transition: "all 0.2s",
                }}>
                  ...
                  {dayEvents.slice(0, 3).map(ev => (
                    <div key={ev.id} style={{
                      fontFamily: font, fontSize: 10, color: palette.text, background: palette.pink,
                      borderRadius: 6, padding: "2px 6px", marginBottom: 2, overflow: "hidden",
      ...
      {!loading && events.length > 0 && (
        <div style={{ marginTop: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {events.map(ev => (
              <Card key={ev.id} style={{ padding: "12px 18px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 4, height: 36, borderRadius: 4, background: palette.lilacDark, flexShrink: 0 }} />
                  <div>
...
function WeekEventsWidget({ accessToken }) {
  ...
  if (!accessToken) return (
    <Card>
      <h4 style={{ margin: "0 0 10px 0", fontFamily: font, color: palette.text, fontSize: 15 }}>📅 Agenda da Semana</h4>
      <div style={{ fontFamily: font, color: palette.textMuted, fontSize: 13, textAlign: "center", padding: 12 }}>Conecte sua conta Google 🌙</div>
    </Card>
  );
  ...
  if (loading) return (
    <Card>
      <h4 style={{ margin: "0 0 10px 0", fontFamily: font, color: palette.text, fontSize: 15 }}>📅 Agenda da Semana</h4>
      <div style={{ fontFamily: font, color: palette.textMuted, fontSize: 13, textAlign: "center", padding: 12 }}>Carregando...</div>
    </Card>
  );
  ...
  const grouped = {};
  events.forEach(ev => {
    const key = new Date(ev.start).toLocaleDateString("pt-BR", { weekday: "long", day: "2-digit", month: "2-digit" });
            ...
            <div key={day}>
              ...
              {dayEvents.map(ev => (
                <div key={ev.id} style={{ display: "flex", alignItems: "center", gap: 8, background: palette.bg, borderRadius: 10, padding: "8px 12px", marginBottom: 4 }}>
                  <div style={{ width: 3, height: 24, borderRadius: 3, background: palette.pinkDark, flexShrink: 0 }} />
                  <div>
                    <div style={{ fontFamily: font, fontSize: 13, fontWeight: 600, color: palette.text }}>{ev.title}</div>
                    <div style={{ fontFamily: font, fontSize: 11, color: palette.textMuted }}>
  ...
  return (
    <div>
      ...
      {subjects.length === 0 ? <EmptyState icon="🎓" text="Nenhuma matéria cadastrada ainda..." /> :
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {subjects.map(s => { const progress = getSubjectProgress(s); const isOpen = expandedSubject === s.id; return (
            <Card key={s.id}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ cursor: "pointer", flex: 1 }} onClick={() => setExpandedSubject(isOpen ? null : s.id)}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: 12, transition: "transform 0.2s", display: "inline-block", transform: isOpen ? "rotate(90deg)" : "rotate(0)" }}>▶</span><div style={{ fontWeight: 700, color: palette.text, fontFamily: font, fontSize: 15 }}>{s.name}</div>{progress === 100 && <span style={{ fontSize: 14 }}>✅</span>}</div>
                  <div style={{ marginTop: 8, maxWidth: 400 }}><ProgressBar percent={progress} size="small" /></div>
                </div>
                <DeleteBtn onClick={() => deleteSubject(s.id)} />
              </div>
              {isOpen && (<div style={{ marginTop: 16, paddingLeft: 8 }}>
                {s.classes.map(c => { const classDone = c.videos.filter(v => v.done).length; return (
                  <div key={c.id} style={{ marginBottom: 14 }}>
                    <div style={{ fontFamily: font, fontSize: 13, fontWeight: 700, color: palette.textLight, marginBottom: 6 }}>📘 {c.name} <span style={{ fontWeight: 500, color: palette.textMuted }}>({classDone}/{c.videos.length} vídeos)</span></div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, paddingLeft: 8 }}>
                      {c.videos.map(v => (<button key={v.id} onClick={() => toggleVideo(s.id, c.id, v.id)} style={{ fontFamily: font, border: `2px solid ${v.done ? palette.greenDark : palette.border}`, borderRadius: 10, padding: "6px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer", background: v.done ? palette.green : "#fff", color: v.done ? palette.greenDark : palette.textMuted, transition: "all 0.2s" }}>{v.done ? "✓ " : ""}{v.name}</button>))}
                    </div>
                  </div>); })}
                {s.hasExam !== false && (<div style={{ marginTop: 8, borderTop: `1px solid ${palette.border}`, paddingTop: 12, display: "flex", alignItems: "center", gap: 10 }}>
                  <button onClick={() => toggleExam(s.id)} style={{ fontFamily: font, border: `2px solid ${s.examDone ? palette.greenDark : palette.border}`, borderRadius: 12, padding: "8px 16px", fontSize: 13, fontWeight: 700, cursor: "pointer", background: s.examDone ? palette.green : "#fff", color: s.examDone ? palette.greenDark : palette.textMuted, transition: "all 0.2s", display: "flex", alignItems: "center", gap: 6 }}>{s.examDone ? "✅" : "📝"} Prova {s.examDone ? "(Concluída)" : "(Pendente)"}</button>
...
function CursosPage({ data, setData }) {
  ...
  return (
    <div>
      {courses.length > 0 && <Card style={{ marginBottom: 20, background: "linear-gradient(135deg, #dbeafe, #fce7f3)" }}><div style={{ fontFamily: font, fontSize: 14, fontWeight: 700, color: palette.text, marginBottom: 8 }}>📊 Progresso Geral dos Cursos</div><ProgressBar percent={getTotalProgress()} /></Card>}
      ...
      {courses.length === 0 ? <EmptyState icon="📚" text="Nenhum curso cadastrado..." /> :
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {courses.map(c => { const progress = getCourseProgress(c); const isOpen = expandedCourse === c.id; return (
            <Card key={c.id}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ cursor: "pointer", flex: 1 }} onClick={() => setExpandedCourse(isOpen ? null : c.id)}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: 12, transition: "transform 0.2s", display: "inline-block", transform: isOpen ? "rotate(90deg)" : "rotate(0)" }}>▶</span><div style={{ fontWeight: 700, color: palette.text, fontFamily: font, fontSize: 15 }}>{c.name}</div>{progress === 100 && <span style={{ fontSize: 14 }}>🎉</span>}</div>
              ...
              {isOpen && (<div style={{ marginTop: 16, paddingLeft: 8 }}>
                {c.modules.map(m => { const mDone = m.lessons.filter(l=>l.done).length; return (
                  <div key={m.id} style={{ marginBottom: 14 }}>
                    <div style={{ fontFamily: font, fontSize: 13, fontWeight: 700, color: palette.textLight, marginBottom: 6 }}>📦 {m.name} <span style={{ fontWeight: 500, color: palette.textMuted }}>({mDone}/{m.lessons.length} aulas)</span></div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, paddingLeft: 8 }}>
                      {m.lessons.map(l => (<button key={l.id} onClick={() => toggleLesson(c.id, m.id, l.id)} style={{ fontFamily: font, border: `2px solid ${l.done ? palette.greenDark : palette.border}`, borderRadius: 10, padding: "6px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer", background: l.done ? palette.green : "#fff", color: l.done ? palette.greenDark : palette.textMuted, transition: "all 0.2s" }}>{l.done ? "✓ " : ""}{l.name}</button>))}
                    </div>
                  </div>); })}
              </div>)}
            </Card>); })}
...
/* ─── Library Page ─── */
const GENRES = [
  { id: "ficcao", label: "Ficção", color: "#c4b5fd" }, { id: "romance", label: "Romance", color: "#fbcfe8" },
  { id: "autoajuda", label: "Autoajuda", color: "#a7f3d0" }, { id: "negocios", label: "Negócios", color: "#93c5fd" },
  { id: "biografia", label: "Biografia", color: "#fde68a" }, { id: "fantasia", label: "Fantasia", color: "#ddd6fe" },
  { id: "suspense", label: "Suspense", color: "#fca5a5" }, { id: "psicologia", label: "Psicologia", color: "#a5f3fc" },
  { id: "educacao", label: "Educação", color: "#bfdbfe" }, { id: "outro", label: "Outro", color: "#e0f2fe" },
];
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        {GENRES.map(g => { const c = books.filter(b=>b.genre===g.id).length; if(c===0) return null; return <button key={g.id} onClick={() => setGenreFilter(g.id)} style={{ fontFamily: font, border: "none", cursor: "pointer", borderRadius: 10, padding: "6px 12px", fontSize: 11, fontWeight: 600, background: genreFilter===g.id ? g.color : "#f5f3ff", color: genreFilter===g.id ? palette.text : palette.textMuted, transition: "all 0.2s" }}>{g.label} ({c})</button>; })}
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}><h3 style={{ margin: 0, color: palette.text, fontFamily: font, fontSize: 17 }}>📖 Minha Biblioteca</h3><Btn onClick={() => setModal("add")} small>＋ Adicionar livro</Btn></div>
      {filtered.length === 0 ? <EmptyState icon="📖" text="Nenhum livro aqui ainda..." /> :
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 12 }}>
          {filtered.map(b => { const st = BOOK_STATUSES.find(s=>s.id===b.status); const genre = GENRES.find(g=>g.id===b.genre); return (
            <Card key={b.id} style={{ padding: "18px 20px", position: "relative" }}>
              ...
              <button onClick={() => cycleStatus(b.id)} style={{ display: "inline-block", marginTop: 10, background: st.color, color: palette.text, fontFamily: font, fontSize: 12, fontWeight: 700, borderRadius: 10, padding: "5px 12px", border: "none", cursor: "pointer", transition: "all 0.2s" }}>{st.icon} {st.label}</button>
      ...
      <Modal open={modal === "add"} onClose={() => { setModal(null); setForm({}); }} title="Adicionar Livro"><div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
    ...
    <div>
      ...
      {filtered.length === 0 ? <EmptyState icon="🔗" text="Nenhum link salvo ainda..." /> :
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 12 }}>
          {filtered.map(l => { const cat = CATEGORIES.find(c=>c.id===l.category)||CATEGORIES[3]; return (
      ...
      <Modal open={modal === "add"} onClose={() => { setModal(null); setForm({}); }} title="Novo Link Útil"><div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>{CATEGORIES.map(c => (<button key={c.id} onClick={() => setForm({...form,category:c.id})} style={{ flex: 1, minWidth: 80, fontFamily: font, border: `2px solid ${(form.category||"outros")===c.id ? palette.lilacDark : "transparent"}`, borderRadius: 12, padding: "8px", fontSize: 12, fontWeight: 600, background: c.color, color: palette.text, cursor: "pointer", transition: "all 0.2s" }}>{c.icon} {c.label}</button>))}</div>
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12, marginBottom: 24 }}>
        {stats.map(s => (<Card key={s.label} style={{ padding: "18px", textAlign: "center", background: s.color }}><div style={{ fontSize: 28 }}>{s.icon}</div><div style={{ fontFamily: font, fontSize: 22, fontWeight: 800, color: palette.text, marginTop: 4 }}>{s.value}</div><div style={{ fontFamily: font, fontSize: 12, color: palette.textLight, fontWeight: 600 }}>{s.label}</div></Card>))}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        <Card>
          ...
          {pendingSubjects.length === 0 ? <div style={{ fontFamily: font, color: palette.textMuted, fontSize: 13, textAlign: "center", padding: 16 }}>Tudo concluído! 🎉</div> :
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>{pendingSubjects.slice(0,5).map(s => (<div key={s.id} style={{ background: palette.bg, borderRadius: 12, padding: "10px 14px" }}><div style={{ fontFamily: font, fontSize: 13, fontWeight: 600, color: palette.text, marginBottom: 6 }}>{s.name}</div><ProgressBar percent={getSubjectProgress(s)} size="small" /></div>))}</div>}
        </Card>
        <Card>
...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div style={{
      ...
      background: "linear-gradient(135deg, #faf5ff 0%, #fce7f3 50%, #eff6ff 100%)", fontFamily: font,
    }}>
      ...
      <div style={{ background: "#fff", borderRadius: 28, padding: "48px 40px", textAlign: "center", boxShadow: "0 20px 60px rgba(124,58,237,0.12)", maxWidth: 380, width: "90%" }}>
        ...
        <button onClick={handleGoogleLogin} disabled={loading} style={{
          fontFamily: font, border: `2px solid ${palette.border}`, cursor: loading ? "wait" : "pointer", fontWeight: 700,
          borderRadius: 14, padding: "12px 24px", fontSize: 15, width: "100%",
          background: "#fff", color: palette.text, transition: "all 0.2s",
...
export default function App() {
  ...
  const handleLogout = async () => { await signOut(auth); setUser(null); setAccessToken(""); setLoaded(false); };
  ...
  if (!authChecked) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: palette.bg, fontFamily: font, color: palette.textLight, fontSize: 18 }}>
      <div style={{ textAlign: "center" }}><div style={{ fontSize: 48, marginBottom: 12, animation: "pulse 1.5s infinite" }}>✨</div>Carregando...</div>
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: palette.bg, fontFamily: font }}>
      ...
      {sidebarOpen && (
        <div style={{ width: 220, background: "#fff", borderRight: `1px solid ${palette.border}`, padding: "24px 16px", display: "flex", flexDirection: "column", gap: 6, position: "sticky", top: 0, height: "100vh", flexShrink: 0, animation: "slideIn 0.2s ease" }}>
          ...
          <div style={{ flex: 1 }} />
          <button onClick={handleLogout} style={{ fontFamily: font, border: "none", cursor: "pointer", borderRadius: 12, padding: "10px 14px", fontSize: 12, fontWeight: 600, background: palette.bg, color: palette.textMuted, transition: "all 0.2s", textAlign: "left" }}>
            🚪 Sair
          </button>
