    ...
    <div>
      ...
      {filtered.length === 0 ? <EmptyState icon="📖" text="Nenhum livro aqui ainda..." /> :
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 12 }}>{filtered.map(b => { const st = BOOK_STATUSES.find(s=>s.id===b.status); const genre = GENRES.find(g=>g.id===b.genre); return (
          <Card key={b.id} style={{ padding: "18px 20px", position: "relative" }}><div style={{ position: "absolute", top: 12, right: 12 }}><DeleteBtn onClick={() => deleteBook(b.id)} /></div>{genre && <span style={{ display: "inline-block", background: genre.color, color: palette.text, fontFamily: font, fontSize: 10, fontWeight: 700, borderRadius: 8, padding: "2px 8px", marginBottom: 6 }}>{genre.label}</span>}<div style={{ fontWeight: 700, color: palette.text, fontFamily: font, fontSize: 15, paddingRight: 28 }}>{b.title}</div>{b.author && <div style={{ color: palette.textMuted, fontSize: 13, fontFamily: font, marginTop: 2 }}>{b.author}</div>}<div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>{b.pages && <span style={{ fontFamily: font, fontSize: 11, color: palette.textMuted, background: palette.bg, borderRadius: 8, padding: "3px 8px" }}>📄 {b.pages} págs</span>}{b.startDate && <span style={{ fontFamily: font, fontSize: 11, color: palette.textMuted, background: palette.bg, borderRadius: 8, padding: "3px 8px" }}>📅 {b.startDate}</span>}{b.endDate && <span style={{ fontFamily: font, fontSize: 11, color: palette.textMuted, background: palette.bg, borderRadius: 8, padding: "3px 8px" }}>🏁 {b.endDate}</span>}</div><button onClick={() => cycleStatus(b.id)} style={{ display: "inline-block", marginTop: 10, background: st.color, color: palette.text, fontFamily: font, fontSize: 12, fontWeight: 700, borderRadius: 10, padding: "5px 12px", border: "none", cursor: "pointer" }}>{st.icon} {st.label}</button>{b.status === "lido" && (<div style={{ marginTop: 8 }}>{[1,2,3,4,5].map(star => (<span key={star} onClick={() => setRating(b.id, star)} style={{ cursor: "pointer", fontSize: 18, filter: star <= b.rating ? "none" : "grayscale(1) opacity(0.3)" }}>⭐</span>))}</div>)}</Card>); })}</div>}
      <Modal open={modal === "add"} onClose={() => { setModal(null); setForm({}); }} title="Adicionar Livro"><div style={{ display: "flex", flexDirection: "column", gap: 12 }}><Input value={form.title||""} onChange={v => setForm({...form,title:v})} placeholder="Título" /><Input value={form.author||""} onChange={v => setForm({...form,author:v})} placeholder="Autor(a)" /><Input value={form.pages||""} onChange={v => setForm({...form,pages:v})} placeholder="Páginas" type="number" /><div style={{ fontFamily: font, fontSize: 13, fontWeight: 700, color: palette.text }}>Gênero</div><div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{GENRES.map(g => (<button key={g.id} onClick={() => setForm({...form,genre:g.id})} style={{ fontFamily: font, border: `2px solid ${(form.genre||"outro")===g.id ? palette.lilacDark : "transparent"}`, borderRadius: 10, padding: "6px 12px", fontSize: 12, fontWeight: 600, background: g.color, color: palette.text, cursor: "pointer" }}>{g.label}</button>))}</div><div style={{ display: "flex", gap: 8 }}><div style={{ flex: 1 }}><div style={{ fontFamily: font, fontSize: 12, color: palette.textMuted, marginBottom: 4 }}>Início</div><Input value={form.startDate||""} onChange={v => setForm({...form,startDate:v})} placeholder="dd/mm/aaaa" /></div><div style={{ flex: 1 }}><div style={{ fontFamily: font, fontSize: 12, color: palette.textMuted, marginBottom: 4 }}>Fim</div><Input value={form.endDate||""} onChange={v => setForm({...form,endDate:v})} placeholder="dd/mm/aaaa" /></div></div><div style={{ fontFamily: font, fontSize: 13, fontWeight: 700, color: palette.text }}>Status</div><div style={{ display: "flex", gap: 8 }}>{BOOK_STATUSES.map(s => (<button key={s.id} onClick={() => setForm({...form,status:s.id})} style={{ flex: 1, fontFamily: font, border: `2px solid ${(form.status||"quero")===s.id ? palette.lilacDark : "transparent"}`, borderRadius: 12, padding: "8px", fontSize: 12, fontWeight: 600, background: s.color, color: palette.text, cursor: "pointer" }}>{s.icon} {s.label}</button>))}</div><Btn onClick={addBook} style={{ marginTop: 4, alignSelf: "flex-end" }}>Salvar 📖</Btn></div></Modal>
    </div>
  );
}
